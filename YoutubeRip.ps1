...
#Video + Subs
.\yt-dlp.exe --write-sub --convert-subs srt --sub-lang en -f bestvideo[height<=1080][ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best --newline -i -o $path --ignore-config --hls-prefer-native $line

#Just Subs If you want only subs, uncomment this and comment out the above line.
#.\yt-dlp.exe --write-sub --convert-subs srt --sub-lang en --skip-download -o $path --ignore-config $line
